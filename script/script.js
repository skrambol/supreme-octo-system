...
var chancellor = {
    ...
    createTable : function(element, dimensions, sol_flag, _board){
        $(element).empty();
        ...
        function getChancellor(board, i, j) {
            if (board.length) {
                if (board[i][j] == 1) chancellor.count++;
    ...
    createPagination : function(count, callback, el){
        el = el || '#pagination';
        $(el).empty();
        $(el).materializePagination({
            align: 'center',
            lastPage:  count,
            firstPage:  1,
